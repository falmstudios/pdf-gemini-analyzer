...
async function refreshStats() {
    try {
        // --- FIX: Point to the correct builder route ---
        const response = await fetch('/dictionary-builder/stats');
...
async function uploadKrogmannFiles() {
    ...
    for (let file of files) {
        try {
            ...
            // --- FIX: Point to the correct builder route ---
            const response = await fetch('/dictionary-builder/upload-krogmann', {
                method: 'POST',
...
async function processAhrhammar() {
    ...
    try {
        // --- FIX: Point to the correct builder route ---
        const response = await fetch('/dictionary-builder/process-ahrhammar', {
            method: 'POST'
        });
...
async function processKrogmann() {
    ...
    try {
        // --- FIX: Point to the correct builder route ---
        const response = await fetch('/dictionary-builder/process-krogmann', {
            method: 'POST'
        });
...
async function processCSV() {
    ...
    try {
        // --- FIX: Point to the correct builder route ---
        const response = await fetch('/dictionary-builder/process-csv', {
            method: 'POST'
        });
...
async function monitorProgress(processType) {
    try {
        // --- FIX: Point to the correct builder route ---
        const response = await fetch(`/dictionary-builder/progress/${processType}`);
...
function showResults(results) {
    ...
    const html = `
        <div class="result-item"><span class="result-label">Total Records Processed:</span><span class="result-value">${results.totalProcessed || 0}</span></div>
        <div class="result-item"><span class="result-label">Concepts Created/Updated:</span><span class="result-value">${results.conceptsCreated || 0}</span></div>
        <div class="result-item"><span class="result-label">Terms Created/Updated:</span><span class="result-value">${results.termsCreated || 0}</span></div>
        <div class="result-item"><span class="result-label">Examples Added:</span><span class="result-value">${results.examplesCreated || 0}</span></div>
        <div class="result-item"><span class="result-label">Relations Created:</span><span class="result-value">${results.relationsCreated || 0}</span></div>
        <div class="result-item"><span class="result-label">Processing Time:</span><span class="result-value">${results.processingTime || 'N/A'}</span></div>
    `;
...
async function clearDictionary() {
    if (!confirm('Are you sure you want to delete ALL dictionary data? This cannot be undone!')) return;
    if (!confirm('This will delete all concepts, terms, translations, examples, and relations. Are you REALLY sure?')) return;
    try {
        // --- FIX: Point to the correct builder route ---
        const response = await fetch('/dictionary-builder/clear-all', { method: 'POST' });
        if (response.ok) {
